...
var userSaidYes = JSON.parse(localStorage.getItem('userSaidYes'));

if (!hasShownWelcomeMessage || !consent) {
  // Show the welcome message and set the flag in local storage
  var result = confirm("This website is very funny 😄 . If you open it, you won't be able to resist smiling! and also fall in love 💌 with Shubham OR you may abuse him !! So you have to open this at your risk!! well your not gonna visit this website so its doesn't matter");
...
if (userSaidYes) {
  // Skip the yes/no question part if user has already said yes
  document.getElementById('yes-no').classList.add('unvis');
  document.getElementById('next-part').classList.remove('unvis');
}
